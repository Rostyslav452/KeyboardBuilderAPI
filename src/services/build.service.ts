...
class BuildService {
   ...
   async createBuild(data: CreateBuildDto, username: string): Promise<Build> {
      const newBuild = await db.Build.create({ ...data, username });

      return await this.getBuildById(newBuild.id);
   }
   ...
   async updateBuild(id: string, updatedData: UpdateBuildDto, username: string): Promise<Build> {
      ...
      await buildInstance.update(updatedData);

      return await this.getBuildById(id);
   }
