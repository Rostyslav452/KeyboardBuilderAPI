...
module.exports = {
    up: async (queryInterface, Sequelize) => {
        ...
        const parts = await queryInterface.sequelize.query(
            `SELECT id, name, type FROM parts WHERE name IN (:partNames)`,
            {
        ...
        const partMap = new Map(parts.map(part => [`${part.type}:${part.name}`, part.id]));
        ...
        const existingBuildNames = new Set(existingBuilds.map(build => build.name));
    ...
    down: async (queryInterface, Sequelize) => {
        return queryInterface.bulkDelete(
            ...
            {
                name: {
                    [Sequelize.Op.in]: buildDefinitions.map(build => build.name),
                },
            },
            {},
        );
